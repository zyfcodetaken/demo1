...
function addFood(){
	...
	if($res&&$nid){
		...
		$mes="<p>添加成功!</p><a href='addFood.php' target='mainFrame'>继续添加</a>|<a href='listFood.php' target='mainFrame'>查看商品列表</a>";
	}else{
		foreach($uploadFiles as $uploadFile){
			if(file_exists("../image_800/".$uploadFile['name'])){
				unlink("../image_800/".$uploadFile['name']);
		...
		$mes="<p>添加失败!</p><a href='addFood.php' target='mainFrame'>重新添加</a>";
...
function delFood($id){
	$where="id=$id";
	$res=delete("imooc_food",$where);
	$proImgs=getAllImgByFoodId($id);
	if($proImgs&&is_array($proImgs)){
		foreach($proImgs as $proImg){
			if(file_exists("uploads/".$proImg['albumPath'])){
				unlink("uploads/".$proImg['albumPath']);
	...
	$res1=delete("imooc_album",$where1);
	if($res&&$res1){
		$mes="删除成功!<br/><a href='listFood.php' target='mainFrame'>查看商品列表</a>";
	}else{
		$mes="删除失败!<br/><a href='listFood.php' target='mainFrame'>重新删除</a>";
	}
	return $mes;
}
/**
...
function getAllImgByFoodId($id){
	$sql="select a.albumPath from imooc_album a where nid={$id}";
	$rows=fetchAll($sql);
	return $rows;
}
...
function getProFoods(){
		$sql="select imooc_food.foodname,imooc_food.content,imooc_album.albumPath from imooc_food,imooc_album where imooc_food.id=imooc_album.nid order by imooc_food.id asc";
		$rows=fetchAll($sql);
		return $rows;
}
